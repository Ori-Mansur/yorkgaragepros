...
export const generateAndUploadPDF = async (formData, totals) => {
  ...
  // 5. TOTALS SECTION (UPDATED WITH TAX)
  const pageHeight = doc.internal.pageSize.height;
  let finalY = Math.max(doc.lastAutoTable.finalY + 30, pageHeight / 2 + 100);
  ...
  // Tax (Added Line)
  doc.text(`Tax (HST ${formData.taxRate}%):`, 365, finalY + 40);
  doc.text(`$${totals.tax.toFixed(2)}`, 550, finalY + 40, { align: "right" });

  // Grand Total Box (Shifted Y down to accommodate tax line)
  doc.setFillColor(primaryAmber[0], primaryAmber[1], primaryAmber[2]);
  doc.roundedRect(350, finalY + 55, 210, 45, 8, 8, "F");
  doc.setFont("Montserrat-Bold", "bold").setFontSize(12).setTextColor(deepSlate[0]);
  
  const totalLabel = formData.type === "RECEIPT" ? "TOTAL PAID" : 
                     formData.type === "QUOTE" ? "ESTIMATED TOTAL" : "TOTAL DUE";
  doc.text(totalLabel, 365, finalY + 83);
  doc.text(`$${totals.grandTotal.toFixed(2)}`, 550, finalY + 83, { align: "right" });
  ...
  const uploadData = { 
    ...
    taxAmount: totals.tax // Ensuring tax info is saved to cloud
  };
  return await saveDocToCloud(uploadData, pdfBlob, fileName);
};